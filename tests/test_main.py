...
from fate.main import _parse_duration, _venv_setting, cmd_run, init_repo
...
def test_cmd_run_never_pushes(repo, monkeypatch):
    root = Path(repo.working_tree_dir)
    (root / ".faterc").write_text("[actions]\npush = { enabled = true }\n")
    seen = {}
    monkeypatch.setattr(
        "fate.main.run_repo", lambda entry, tasks: seen.update(tasks=tasks)
    )
    cmd_run(argparse.Namespace(directory=str(root)))
    assert "push" not in seen["tasks"]
